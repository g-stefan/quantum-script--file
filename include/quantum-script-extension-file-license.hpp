//
// Quantum Script Extension File
//
// Copyright (c) 2020-2021 Grigore Stefan <g_stefan@yahoo.com>
// Created by Grigore Stefan <g_stefan@yahoo.com>
//
// MIT License (MIT) <http://opensource.org/licenses/MIT>
//

#ifndef QUANTUM_SCRIPT_EXTENSION_FILE_LICENSE_HPP
#define QUANTUM_SCRIPT_EXTENSION_FILE_LICENSE_HPP

#ifndef QUANTUM_SCRIPT_EXTENSION_FILE__EXPORT_HPP
#include "quantum-script-extension-file--export.hpp"
#endif

namespace Quantum {
	namespace Script {
		namespace Extension {
			namespace File {
				namespace License {
					QUANTUM_SCRIPT_EXTENSION_FILE_EXPORT const char *content();
					QUANTUM_SCRIPT_EXTENSION_FILE_EXPORT const char *shortContent();
				};
			};
		};
	};
};

#endif
